...
void* GetFunction(void* pthis, int id, bool& istramp)
{
	...
	for (size_t i = 0; i < g_Hooks[id].length(); ++i)
	{
		if (func == g_Hooks[id].at(i)->tramp)
		{
			istramp = true;
			return func;
		}
...
int RegisterHam(int func, const char* classname, void* targetfunc)
{
	...
	void** vtable = GetVTable(pEntity->pvPrivateData, Offsets.GetBase());
	REMOVE_ENTITY(pEntity);
	...
	*/
	...
	Hook* hook = new Hook(vtable, hooklist[func].vtid, targetfunc, hooklist[func].isvoid, hooklist[func].needsretbuf, hooklist[func].paramcount, classname);
