...
EventsMngr::ClEvent::ClEvent(void (*func)(int), int flags)
{
	m_Func = func;
...
void (*EventsMngr::ClEvent::getFunction(int))(int)
{
	return m_Func;
}
...
EventsMngr::ClEvent *EventsMngr::registerEvent(void(*func)(int), int flags, int msgid)
{
	...
	auto event = ke::AutoPtr<ClEvent>(new ClEvent(func, flags));

	m_Events[msgid].append(ke::Move(event));

	return m_Events[msgid].back();
}

void EventsMngr::parserInit(int msg_type, float* timer, edict_t* pPlayer, int index)
{
	...
	for (auto& event : m_Events[msg_type])
	{
		...
		if (pPlayer)
		{
			int playerIndex = g_TypeConversion.edict_to_id(pPlayer);
			if (!event->m_FlagClient || (MF_IsPlayerBot(playerIndex) ? !event->m_FlagBot : !event->m_FlagPlayer) || (MF_IsPlayerAlive(playerIndex) ? !event->m_FlagAlive : !event->m_FlagDead))
			{
				event->m_Done = true;
				continue;
			}
...
void RegisterEvent(const char* pszEventName, void (*func)(int), const char* pszFlags, int n_args, ...)
{
	int flags = UTIL_ReadFlags(pszFlags);

	int eventId;
	if ((eventId = g_events.getEventId(pszEventName)) == 0)
	{
		MF_Log("Invalid event (name \"%s\")", pszEventName);
		return;
	}

	auto pEvent = g_events.registerEvent(func, flags, eventId);
	...
	{
		char szFilter[100];
		const char* pszStr = va_arg(ap, const char*);
		strcpy(szFilter, pszStr);

		pEvent->registerFilter(szFilter);
	}
