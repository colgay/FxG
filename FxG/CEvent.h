...
public:
	...
	class ClEvent
	{
		...
		void (*m_Func)(int);								// function to be executed
	...
	public:
		...
		ClEvent(/*CPluginMngr::CPlugin* plugin, */void (*func)(int), int flags);
		~ClEvent();
...
public:
	...
	EventsMngr::ClEvent *registerEvent(/*CPluginMngr::CPlugin* plugin, */void(*func)(int), int flags, int msgid);

	void parserInit(int msg_type, float* timer, edict_t *pPlayer, int index);
